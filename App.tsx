...
const App: React.FC = () => {
  const [view, setView] = useState<'chat' | 'training' | 'compliance' | 'history'>('chat');
      ...
      mediaRecorderRef.current.onstop = async () => {
        ...
        reader.onloadend = () => {
          ...
          const userVoiceMsg: Message = {
            ...
            timestamp: Date.now(),
            status: 'sending', // Setting to sending so processAIResponse can confirm it
            voiceUrl: audioUrl,
  ...
  return (
    <div className="flex flex-col h-screen bg-[#f0f7ff] font-sans text-slate-900 overflow-hidden">
      ...
      <main className="flex-1 flex overflow-hidden">
        {view === 'chat' && (
          <div className="flex-1 flex flex-col min-w-0 bg-white md:m-4 md:rounded-[2rem] shadow-xl shadow-blue-100 overflow-hidden border border-blue-50">
        ...
        {view === 'compliance' && (
          <div className="flex-1 p-8 bg-slate-50 overflow-y-auto">
             <div className="max-w-4xl mx-auto">
                <div className="bg-[#1068eb] rounded-3xl p-8 text-white mb-8 shadow-xl shadow-blue-100">
                   <h2 className="text-2xl font-bold mb-2 flex items-center gap-3">Health & Ethics</h2>
                   <p className="text-blue-100 opacity-80 text-sm">MANAS360 operates under strict Tele-MANAS guidelines.</p>
                </div>
             </div>
          </div>
        )}
        ...
        {view === 'chat' && (
          <aside className="hidden xl:flex w-80 flex-col border-l border-blue-50 bg-[#f8fbff] p-6 overflow-y-auto">
            <div className="mb-8">
              <h3 className="font-bold text-slate-800 text-sm mb-4 flex items-center gap-2">
                <PlusCircle size={18} className="text-[#1068eb]" />
                Quick Starters
              </h3>
              <div className="grid grid-cols-1 gap-2">
                {t.starters.map((starter: string, i: number) => (
                  <button 
                    key={i} 
                    onClick={() => { 
                      if (chatSubMode === 'text') {
                        ...
                        if (!checkVoiceLimit(userProfile)) { setShowPremiumModal(true); return; }
                        processAIResponse(starter, true);
                      }
                    }}
                    className="text-left p-3 rounded-2xl bg-white border border-slate-100 text-xs font-semibold text-slate-600 hover:border-[#1068eb] hover:bg-blue-50 transition-all shadow-sm"
                  >
                    {starter}
                  </button>
                ))}
            ...
            <div className="mt-auto">
              <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm mb-4">
                 <p className="text-[10px] font-bold text-slate-400 uppercase mb-2">Support ID</p>
                 <p className="text-xs font-mono font-bold text-slate-700">TM-AS-24-Isolated</p>
              </div>
              {showCrisisResources && <CrisisPanel />}
            </div>
          </aside>
        )}
      </main>
