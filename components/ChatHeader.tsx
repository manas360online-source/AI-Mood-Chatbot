...
import { Shield, Sparkles, Heart, GraduationCap, MessageCircle, LayoutDashboard, Trash2, History } from 'lucide-react';

interface ChatHeaderProps {
  isPremium: boolean;
  onUpgrade: () => void;
  remainingInfo: string;
  view: 'chat' | 'training' | 'compliance' | 'history';
  onViewChange: (view: 'chat' | 'training' | 'compliance' | 'history') => void;
  onClearChat?: () => void;
  usageLabel?: string;
  upgradeLabel?: string;
}
  ...
  return (
    <header className="bg-white border-b border-blue-100 px-4 md:px-6 py-3 flex items-center justify-between sticky top-0 z-10 shadow-sm">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 bg-[#1068eb] rounded-full flex items-center justify-center text-white shadow-md">
          <Heart size={20} fill="currentColor" />
        </div>
        <div className="hidden sm:block">
          <div className="flex items-center gap-2">
            <h1 className="font-bold text-slate-800 text-sm">MANAS360</h1>
            <span className="text-[8px] font-bold text-[#1068eb] bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100 uppercase tracking-tighter">Verified</span>
          </div>
