...
const TrainingPortal: React.FC<TrainingPortalProps> = ({ profile, onUpdateProfile }) => {
  ...
  const completeQuiz = (score: number) => {
    ...
    const certNumber = `MANS360-${modulePrefix}-${year}-${seq}`;
      ...
      <div className="max-w-6xl mx-auto">
                    ...
                    <div key={module.id} className="bg-white rounded-[2rem] border border-slate-100 p-6 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group flex flex-col">
                      ...
                      <div className="mt-auto">
                        ...
                        <button 
                          ...
                          className={`w-full py-3.5 rounded-2xl font-black text-sm tracking-tight transition-all active:scale-95 flex items-center justify-center gap-2 ${
                            isCompleted 
                              ? 'bg-slate-50 text-slate-500 hover:bg-slate-100' 
                              : isFailed ? 'bg-slate-900 text-white' : 'bg-[#1068eb] text-white hover:bg-[#0d56c4] shadow-lg shadow-blue-100'
                          }`}
                        >
            ...
            <div className="bg-white rounded-[3rem] border border-slate-100 shadow-xl overflow-hidden mb-12">
              <div className="p-10 md:p-16">
                <div className="max-w-3xl mx-auto">
                  ...
                  <div className="mt-20 p-10 bg-slate-900 rounded-[2.5rem] text-white shadow-2xl relative overflow-hidden">
                      ...
                      {!profile.certifications.some(c => c.moduleCode === activeModule.code) ? (
                        canRetake(activeModule.id) ? (
                          <button 
                            ...
                            className="bg-[#1068eb] text-white font-black px-10 py-4 rounded-2xl hover:bg-[#0d56c4] shadow-xl shadow-blue-900 transition-all active:scale-95 flex items-center gap-3"
                          >
                          ...
                          <div className="flex flex-col gap-4">
                            <div className="bg-red-500/10 p-5 rounded-2xl border border-red-500/20 flex items-center gap-4">
                              ...
                              <div>
                                <p className="text-sm font-bold text-white uppercase tracking-wider">Cooldown Active</p>
                                <p className="text-xs text-slate-400">Please review the materials. Retake available in {Math.ceil(cooldownTime / 1000)}s.</p>
                              </div>
                            </div>
                          </div>
                        )
                        ...
                        <div className="bg-emerald-500/10 p-5 rounded-2xl border border-emerald-500/20 flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-sm font-bold text-white uppercase tracking-wider">Module Complete</p>
                            <p className="text-xs text-slate-400">You are officially certified in this domain.</p>
                          </div>
                        </div>
                      )}
                    </div>
        ...
        {showCert && (
          <div className="fixed inset-0 z-[100] bg-slate-900/90 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
            <div className="bg-[#0a192f] w-full max-w-5xl rounded-[1rem] shadow-[0_20px_50px_rgba(0,0,0,0.5)] border-[12px] border-slate-800/50 relative animate-in zoom-in-95 duration-500 overflow-hidden group">
               
               {/* Background Texture Curves */}
               <div className="absolute bottom-0 left-0 w-full h-1/2 opacity-10 pointer-events-none">
                 <svg viewBox="0 0 1000 500" className="w-full h-full">
                    <path d="M0 500 C 200 400 400 450 600 350 C 800 250 900 300 1000 200 L 1000 500 Z" fill="#c5a059" />
                 </svg>
               </div>
               ...
               <button 
                ...
                className="absolute top-6 right-6 p-2 text-white/50 hover:text-white bg-white/10 hover:bg-white/20 rounded-full transition-all z-20"
               >
                 <X size={20} />
               </button>
               ...
               <div className="p-12 md:p-16 relative z-10">
                 {/* QR Code Placeholder */}
                 <div className="absolute top-12 right-12 w-16 h-16 bg-white p-1 rounded-sm">
                    <div className="w-full h-full bg-[url('https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=MANAS360-VERIFIED')] bg-cover opacity-80" />
                 </div>

                 {/* Main Content */}
                 <div className="text-center">
                    <h2 className="text-[#c5a059] text-5xl md:text-6xl font-serif font-medium tracking-tight mb-2">CERTIFICATE</h2>
                    <h3 className="text-[#c5a059] text-2xl md:text-3xl font-serif tracking-[0.2em] mb-12">OF COMPLETION</h3>

                    <p className="text-white/80 text-xl font-medium mb-12">This is to certify that</p>
                    
                    <h1 className="text-[#c5a059] text-6xl md:text-8xl font-['Great_Vibes'] mb-12 py-4">
                      {profile.name}
                    </h1>

                    <div className="max-w-2xl mx-auto mb-16">
                      <p className="text-white/70 text-base leading-relaxed inline">
                        has successfully completed the course and awarded by Manas360 Mental Wellness Prvt Ltd., as
                      </p>
                      <h4 className="text-[#c5a059] text-2xl md:text-3xl font-serif mt-6">
                        MANAS360 Certified Therapist
                      </h4>
                    </div>

                    <div className="flex flex-col md:flex-row items-center justify-between gap-12 mt-20 border-t border-white/10 pt-12">
                      <div className="text-left">
                        <p className="text-[#c5a059] text-lg font-serif">Certification ID :</p>
                        <p className="text-white/60 text-sm font-mono mt-1 tracking-wider">{showCert.certNumber}</p>
                      </div>

                      {/* Seal Visualization */}
                      <div className="relative group-hover:scale-110 transition-transform duration-700">
                        <div className="absolute -top-16 -left-16 w-32 h-32 bg-[#c5a059] rounded-full blur-[40px] opacity-20" />
                        <div className="w-32 h-32 bg-[#c5a059] rounded-full border-4 border-white/20 shadow-2xl relative flex items-center justify-center">
                           <div className="w-28 h-28 rounded-full border-2 border-white/30 flex items-center justify-center p-2">
                             <div className="text-[#0a192f] font-black text-[10px] uppercase text-center leading-none">
                               Tele-Manas<br/>Verified<br/>Quality
                             </div>
                           </div>
                           {/* Ribbon Tails */}
                           <div className="absolute -bottom-10 left-4 w-10 h-16 bg-[#c5a059] clip-path-ribbon opacity-90 shadow-lg" style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 50% 80%, 0 100%)' }} />
                           <div className="absolute -bottom-10 right-4 w-10 h-16 bg-[#c5a059] clip-path-ribbon opacity-90 shadow-lg" style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 50% 80%, 0 100%)' }} />
                        </div>
                      </div>

                      <div className="text-center">
                        <div className="mb-2 font-['Great_Vibes'] text-[#c5a059] text-4xl">Howard Ong</div>
                        <div className="w-56 h-[1px] bg-[#c5a059]/40 mb-2" />
                        <p className="text-[#c5a059] text-xs font-serif tracking-widest uppercase">Howard Ong</p>
                        <p className="text-white/50 text-[10px] tracking-tight mt-1">Chair-Clinical Advisory Board</p>
                      </div>
                    </div>
                 </div>
               </div>
            ...
            <div className="mt-8 flex justify-center gap-4">
               <button onClick={() => window.print()} className="px-10 py-4 bg-[#c5a059] text-[#0a192f] rounded-full font-black text-sm shadow-xl hover:bg-[#d4af37] transition-all active:scale-95 flex items-center gap-2">
                 <Printer size={18} />
                 Print Certification
               </button>
            </div>
